  ...
  return <main className="dashboard-page club-page">
    ...
    <section className="club-layout">
      <article className="dashboard-card feature-book">
        ...
        {club.current_book ? <><div className="large-book-spine" /><h2>{club.current_book.title}</h2><p>{club.current_book.authors || "Author unknown"}</p>{club.current_book.suggested_by_display_name && <p className="suggested-by">Suggested by {club.current_book.suggested_by_display_name}</p>}{club.active_cycle?.discussion_date && <p className="date-note">Discussion: {formatLocalDateTime(club.active_cycle.discussion_date)}</p>}{club.upcoming_cycle?.selected_book && <div className="future-cycle-list"><p className="eyebrow">Up next</p><p><strong>{club.upcoming_cycle.name || "Next reading cycle"}</strong> · {club.upcoming_cycle.selected_book.title}</p><p>{club.upcoming_cycle.selected_book.authors || "Author unknown"}</p><p>Discussion: {formatLocalDateTime(club.upcoming_cycle.discussion_date)}</p></div>}{club.future_cycles.length > 0 && <div className="future-cycle-list"><p className="eyebrow">Coming up</p>{club.future_cycles.map((cycle) => <div key={cycle.id}><p><strong>{cycle.name || "Next reading cycle"}</strong></p><p>Suggestions open: {formatLocalDateTime(cycle.suggestion_start_date)}</p><p>Voting opens: {formatLocalDateTime(cycle.voting_start_date)}</p><p>Voting closes: {formatLocalDateTime(cycle.voting_end_date)}</p></div>)}</div>}</> : <><EmptyState text="A new read will be selected when the next cycle begins." /><Link className="quiet-link" to={`/books?clubId=${clubId}`}>Suggest a book →</Link></>}
